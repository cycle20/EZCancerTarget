...
`EZCancerTarget` fetches its input from a simple data source.
It can be a TSV file in case of running scripts on user-controlled computer.
Another option is updating a shared Google Spreadsheet that is processed by
workflow scripts on _GitHub_.

`EZCancerTarget` load data from _clue.io_ [@corsello_drug_2017], then download data from other
sources: _FDA Label_ service, _PubMed®_ - [@ncbi2020] -, _EMA_ - @ema2022 -, _ChEMBL_, _PubChem_ - @pubchem2020.

After data collection it generates a user-friendly report file and some
summary output data file. The report HTML file can be opened by a browser.
If the workflow runs on _GitHub_, it will deploy the report file as
a public GitHub web page.
